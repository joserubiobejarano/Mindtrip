...
export async function generateMetadata({
  ...
  const path = getLocalizedPath("/influencers", lang);
...
export default async function LocalizedInfluencersPage({
  params,
}: {
  params: Promise<{ lang: string }>;
}) {
  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-16">
      <StructuredData data={structuredData} id={`kruno-influencers-list-${lang}`} />
      <div className="space-y-4">
        <h1 className="text-4xl font-bold">{copy.influencersHubTitle}</h1>
        <p className="text-lg text-muted-foreground">{copy.influencersHubSubtitle}</p>
      </div>
      <div className="mt-10 grid gap-6 md:grid-cols-2">
        {influencerPages.map((influencer) => (
          <Link
            key={influencer.slug}
            href={`${basePath}/influencers/${influencer.slug}`}
          ...
          >
            ...
            <div className="mt-4 text-sm text-primary">{copy.influencerCardCta}</div>
          </Link>
        ))}
      </div>
    </div>
