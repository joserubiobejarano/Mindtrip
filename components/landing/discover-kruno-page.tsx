...
export function DiscoverKrunoPage() {
  ...
  const handleStartPlanning = async (event: React.FormEvent) => {
    ...
    if (!selectedDestination) {
      setSearchError(destinationInput.trim() ? "Select a city from the list." : "Enter a destination.");
      return;
    }

    if (!startDate || !endDate) {
      setSearchError("Select your dates.");
      return;
    }
    ...
    <div className="bg-background">
          ...
          <div className="space-y-7 text-center lg:text-left">
            <div className="space-y-3">
              <h1 className="text-4xl md:text-5xl lg:text-6xl font-normal text-foreground font-hand text-balance">
                Planning a trip shouldn&apos;t feel chaotic.
              </h1>
              <p className="text-base md:text-lg text-muted-foreground max-w-xl lg:mx-0 mx-auto">
                Kruno turns a city + dates into a clear itinerary you can actually use and share.
              </p>
            </div>
            ...
            <ul className="space-y-3 text-sm md:text-base text-foreground">
              ...
              <li className="flex items-start justify-center gap-2 lg:justify-start">
                <span className="mt-1 h-2 w-2 rounded-full bg-primary" />
                No bookings. No stress. Just a plan.
              </li>
            </ul>
            ...
            <p className="text-xs text-muted-foreground">
              {displayUtmContent
                ? `Travelers from ${displayUtmContent} get 20% off Kruno Pro.`
                : "Upgrade to Kruno Pro anytime and save 20%."}
            </p>
          </div>
      ...
      <section className="px-6 pb-16">
        <div className="max-w-5xl mx-auto grid gap-6 md:grid-cols-3">
          <div className="bg-card rounded-2xl p-6 shadow-sm">
            ...
            <h3 className="text-lg font-medium mb-2">Pick a city + dates</h3>
            <p className="text-sm text-muted-foreground">Give Kruno the basics and move fast.</p>
          </div>
          <div className="bg-card rounded-2xl p-6 shadow-sm">
            <div className="h-10 w-10 rounded-full bg-accent flex items-center justify-center mb-4">
              <Sparkles className="h-5 w-5 text-muted-foreground" />
            </div>
            <h3 className="text-lg font-medium mb-2">Get a day-by-day itinerary</h3>
            <p className="text-sm text-muted-foreground">A clean plan you can actually follow.</p>
          </div>
          <div className="bg-card rounded-2xl p-6 shadow-sm">
            <div className="h-10 w-10 rounded-full bg-accent flex items-center justify-center mb-4">
              <Share2 className="h-5 w-5 text-muted-foreground" />
            </div>
            <h3 className="text-lg font-medium mb-2">Share it + split expenses</h3>
            <p className="text-sm text-muted-foreground">Invite friends and stay on budget.</p>
          </div>
        </div>
      </section>
      ...
      <section className="px-6 pb-16">
        <div className="max-w-4xl mx-auto bg-secondary/40 rounded-2xl p-6 text-center">
          <p className="text-sm md:text-base text-muted-foreground">
            Used for city breaks, weekends, and first-time visits.
          </p>
        </div>
      </section>
      ...
      <section className="px-6 pb-16">
        <div className="max-w-5xl mx-auto grid gap-6 md:grid-cols-2">
          <div className="bg-card rounded-2xl p-6 shadow-sm">
            <h3 className="text-lg font-medium mb-4">What Kruno is</h3>
            <ul className="space-y-2 text-sm text-muted-foreground">
              <li>A clean trip plan in minutes</li>
              <li>An itinerary + top places + expense split</li>
              <li>Built to reduce stress</li>
            </ul>
          </div>
          <div className="bg-card rounded-2xl p-6 shadow-sm">
            <h3 className="text-lg font-medium mb-4">What Kruno isn&apos;t</h3>
            <ul className="space-y-2 text-sm text-muted-foreground">
              <li>Not a booking platform</li>
              <li>Not a complicated planner</li>
              <li>Not endless research</li>
            </ul>
          </div>
        </div>
      </section>
      ...
      <section className="px-6 pb-20">
        <div className="max-w-4xl mx-auto bg-card rounded-3xl p-8 md:p-10 text-center shadow-lg">
          ...
          <p className="text-xs text-muted-foreground mt-3">Takes a few minutes to generate.</p>
        </div>
      </section>
    </div>
  );
