...
export function TripAssistantWidget({ tripId }: TripAssistantWidgetProps) {
  ...
  // Shared card styling matching homepage
  const cardClassName = "bg-white rounded-3xl border-4 border-black";
  const cardStyle = { boxShadow: '8px 8px 0px rgba(0, 0, 0, 1)' };

  return (
    <div className="sticky bottom-6 w-full max-w-[680px] mx-auto px-4 sm:px-2 z-50 pointer-events-none mt-6">
      <div className={cardClassName} style={cardStyle} onClick={(e) => e.stopPropagation()}>
        {!isExpanded ? (
          ...
          <form
            ...
            className="flex items-center gap-3 px-4 py-3 cursor-pointer pointer-events-auto"
          ...
          >
            ...
            <input
              ref={inputRef}
              className="flex-1 border-none bg-transparent text-sm outline-none placeholder:text-slate-400 pointer-events-auto truncate"
            ...
            <button
              ...
              disabled={sendingChat}
              className="flex-shrink-0 inline-flex items-center justify-center w-8 h-8 rounded-full bg-orange-500 hover:bg-orange-600 text-white disabled:opacity-50 disabled:cursor-not-allowed pointer-events-auto transition-colors"
            >
              {sendingChat ? (
