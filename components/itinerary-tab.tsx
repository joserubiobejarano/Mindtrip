...
export function ItineraryTab({
  ...
  // Initialize aiItinerary from trip data - load immediately when trip data is available
  useEffect(() => {
    if (trip?.itinerary && !aiItinerary && !loadingAiItinerary) {
      setAiItinerary(trip.itinerary);
      hasAutoGenerated.current = true; // Mark as loaded so we don't regenerate
    }
  }, [trip?.itinerary, aiItinerary, loadingAiItinerary]);
  ...
  useEffect(() => {
    if (
      ...
      !aiItinerary &&
      !loadingAiItinerary &&
      !trip.itinerary && // Check if trip already has itinerary
  ...
  // Load existing itinerary from trip data on mount
  useEffect(() => {
    if (trip?.itinerary && !aiItinerary && !loadingAiItinerary) {
      setAiItinerary(trip.itinerary);
    }
  ...
  return (
    <div className="p-6 h-full flex flex-col overflow-hidden bg-gray-50/50">
      ...
      <div className="flex-1 overflow-y-auto pr-2">
                        ...
                        <div key={sectionIdx} className="relative pl-4 border-l-2 border-gray-100">
                          ...
                          <div className="space-y-3">
                            {section.suggestions.map((suggestion, sugIdx) => {
                              ...
                              return (
                                <div key={sugIdx} className="flex items-start gap-3 p-3 rounded-lg hover:bg-gray-50 transition-colors group">
                                  {photoUrl ? (
        ...
        {/* Chat Dialog */}
        {chatOpen && (
          <Card className="fixed bottom-6 right-6 w-[350px] h-[500px] shadow-2xl flex flex-col z-50 animate-in slide-in-from-bottom-10">
            <CardHeader className="bg-primary text-primary-foreground p-4 flex flex-row items-center justify-between rounded-t-lg">
              <div>
                <CardTitle className="text-base">Trip Assistant</CardTitle>
