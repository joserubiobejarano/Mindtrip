...
export function ItineraryTab({
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (itinerary) {
            ...
            return; // Found existing itinerary, don't generate new one
          }
        }
        // If no itinerary exists, continue to auto-generate below
      } catch (error) {
        console.error("Error loading existing itinerary:", error);
        // Continue to generate new one if needed
      } finally {
        setLoadingAiItinerary(false);
      }
    };
  ...
  const generateItinerary = useCallback(async () => {
    ...
    try {
      const checkResponse = await fetch(`/api/trips/${tripId}/smart-itinerary`);
      if (checkResponse.ok) {
        const { itinerary } = await checkResponse.json();
        if (itinerary) {
          setAiItinerary(itinerary);
          hasAutoGenerated.current = true;
          return; // Already exists, don't generate
        }
      }
    } catch (error) {
